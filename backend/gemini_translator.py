...
[텍스트 타입 분류]
각 텍스트 항목에 반드시 "type" 필드를 지정하세요:
- "dialogue"  — 말풍선(타원/구름 등) 안의 대사
- "sfx"       — 말풍선 밖의 효과음 (ドン, ガーン, キャー 등). 크게 그려지거나 기울어지거나 여러 곳에 흩어진 텍스트
- "narration" — 직사각형 박스(내레이션 박스)의 텍스트
- "title"     — 챕터/장면 제목

[sfx 특별 규칙]
- 효과음은 페이지 전체에 크게, 비스듬히, 변형된 형태로 흩어질 수 있습니다
- 글자 덩어리가 공간적으로 분리되어 있으면 각각 별도 항목으로 추출
- sfx의 x/y/width/height = 글리프(글자) 자체를 감싸는 타이트한 바운딩 박스 (말풍선 면적이 아님)
- sfx의 polygon = 실제 글자 획을 따라 타이트하게 감싸는 4~8개 꼭짓점

[좌표 정확도 규칙]
모든 좌표는 이미지 width/height 대비 0.0~1.0 비율:
- dialogue:  x/y/width/height = 말풍선 테두리를 제외한 내부 전체 영역
- sfx:       x/y/width/height = 텍스트 글리프만을 감싸는 타이트한 박스
- narration: x/y/width/height = 내레이션 박스 내부 전체 영역
- polygon:   말풍선/박스/글리프의 실제 외곽선을 따라 4~8개 꼭짓점으로 추적 (단순 직사각형이 아닌 실제 형태)
...
[
  {
    ...
    "translated": "오빠...",
    "type": "dialogue",
    "x": 0.1, "y": 0.18, "width": 0.15, "height": 0.14,
    "direction": "vertical",
  ...
  },
  {
    ...
    "translated": "쾅",
    "type": "sfx",
    "x": 0.4, "y": 0.05, "width": 0.12, "height": 0.08,
    "direction": "horizontal",
    "polygon": [[0.4,0.05],[0.52,0.05],[0.52,0.13],[0.4,0.13]],
    "text_color": "#ffffff",
    "bg_type": "none"
  }
]
